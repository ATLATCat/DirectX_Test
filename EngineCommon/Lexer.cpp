...
ASEParser::Lexer::Lexer()
{
	_tokens.reserve(TOKENR_END);
	for (int i = 0; i < TOKENR_END; ++i)
	{
		_tokens[Token[i]] = static_cast<ASEToken>(i);
	}
}

ASEParser::ASEToken ASEParser::Lexer::GetToken(std::string& tokenString)
{
	if (tokenString[0] == '*' && _tokens.find(tokenString) != _tokens.end())
	{
		return _tokens[tokenString];
	}
	return ASEToken::TOKENR_END;
}
